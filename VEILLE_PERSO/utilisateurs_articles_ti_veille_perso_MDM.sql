CREATE OR REPLACE FUNCTION ti_veilleperso.sp_ti_veille_perso_v1(nbdays integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
declare
...
and gtca.date_modification > CURRENT_DATE-nbdays;
...
where gtut.tag_id is not null and gtut.b_batchid = (select MAX(WEKA_MDM.GD_TI_UTILISATEURS_THESAURUS.b_batchid) from WEKA_MDM.GD_TI_UTILISATEURS_THESAURUS)
and gtca.date_publication > CURRENT_DATE-nbdays;
...
-- Insérer les mails et id tag provenant des 2 tables tmp_user_actu et tmp_user_infocoll dans la table tmp_user_tag, cela permet d'avoir une liste tag/ utilisateur
...
-- mettre à jour la colonne id_unique_actu de la table utilisateurs articles Ti veille perso avec la colonne actu agrégée de la table tmp_user_actu pour chaque utilisateur t1 utilisateur = t2 mails
...
-- supprimer la table tmp_user_infocoll
drop table if exists tmp_user_infocoll;

-- supprimer la table tmp_user_actu
drop table if exists tmp_user_actu;

-- supprimer la table tmp_user_tag
drop table if exists tmp_user_tag;

select count(*) into retval
from TI_VEILLEPERSO.utilisateurs_articles_ti_veille_perso uatvp
WHERE CONTENUS_A_ENVOYER=true;
  return retval;
